...
sfSprite *dash_sprite_create(sfTexture *tex, sfIntRect *rect)
{
    sfSprite *spr = sfSprite_create();
    sfSprite_setTexture(spr, tex, sfTrue);
    if (rect != NULL)
        sfSprite_setTextureRect(spr, *rect);
    sfSprite_setOrigin(spr, (sfVector2f){96, 96});
    return spr;
}

sfSprite *dash_sprite_create_from_file(char *path)
{
    sfTexture *tex = sfTexture_createFromFile(path, NULL);
    sfSprite *spr = dash_sprite_create(tex, NULL);
    return spr;
}
...
int amnt, int h)
{
    ...
    for (i = 0; i < amnt; i++) {
        ...
        sprarr[i] = dash_sprite_create(tex, &rect_c);
    }
    sprarr[i] = NULL;
    return sprarr;
}

sfSprite ***dash_spritesheet_load(animd_t *animd)
{
    ...
    int i = 0;
    for (i = 0; i < AC; i++) {
        sprarr[i] = dash_sprite_array_load(tex, rect, AF[i], i);
    }
    sprarr[i] = NULL;
    return sprarr;
}
